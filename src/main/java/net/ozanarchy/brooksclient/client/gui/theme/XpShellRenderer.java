...
public final class XpShellRenderer {
    ...
    public void renderDesktop(GuiGraphicsExtractor graphics, int width, int height) {
        ...
        assets.fillVerticalGradient(graphics, 0, 0, width, horizon, XpShellAssets.DESKTOP_SKY_TOP, XpShellAssets.DESKTOP_SKY_BOTTOM);
        assets.fillVerticalGradient(graphics, 0, horizon, width, height, XpShellAssets.DESKTOP_FIELD_TOP, XpShellAssets.DESKTOP_FIELD_BOTTOM);
        renderTaskbarBase(graphics, width, height);
    }

    public void renderWallpaperDesktop(GuiGraphicsExtractor graphics, int width, int height, Identifier wallpaperTexture) {
    ...
    public void renderWindowFrame(GuiGraphicsExtractor graphics, int x, int y, int width, int height, String title, Font font) {
        ...
        int titleHeight = 24;
        assets.fillHorizontalGradient(graphics, x + 1, y + 1, x + width - 1, y + titleHeight, XpShellAssets.TITLE_LEFT, XpShellAssets.TITLE_RIGHT);
    ...
    public void renderTaskPane(GuiGraphicsExtractor graphics, int x, int y, int width, int height, String header, Font font) {
        assets.drawRaisedBox(graphics, x, y, x + width, y + height, XpShellAssets.PANEL_MID);
        assets.fillVerticalGradient(graphics, x + 1, y + 1, x + width - 1, y + height - 1, XpShellAssets.TASK_PANE_TOP, XpShellAssets.TASK_PANE_BOTTOM);
        assets.fillVerticalGradient(graphics, x + 1, y + 1, x + width - 1, y + 22, XpShellAssets.TASK_PANE_HEADER, 0xFF1B418F);
    ...
    public void renderListRow(GuiGraphicsExtractor graphics, Font font, int x, int y, int width, int height, String text, boolean selected, boolean enabled) {
    ...
    public void renderStartButton(GuiGraphicsExtractor graphics, Font font, UiRect bounds, boolean hovered, boolean open) {
    ...
    public void renderTaskbarTray(GuiGraphicsExtractor graphics, Font font, UiRect bounds, String clockText) {
        assets.fillVerticalGradient(graphics, bounds.x(), bounds.y(), bounds.right(), bounds.bottom(), 0xFF2E78D8, 0xFF1D58B8);
    ...
    public void renderTaskbarWindowButton(GuiGraphicsExtractor graphics, Font font, UiRect bounds, String label, boolean active, boolean minimized) {
    ...
    public void renderStartMenuShell(GuiGraphicsExtractor graphics, UiRect bounds, UiRect bodyBounds, UiRect actionBounds) {
        ...
        assets.drawRaisedBox(graphics, bounds.x() + 1, bounds.y() + 1, bounds.right() - 1, bounds.bottom() - 1, 0xFFF6F2E9);
        graphics.fill(bodyBounds.x(), bodyBounds.y(), bodyBounds.right(), bodyBounds.bottom(), 0xFFF3EEDF);
        assets.fillVerticalGradient(graphics, actionBounds.x(), actionBounds.y(), actionBounds.right(), actionBounds.bottom(), 0xFFD7E6FF, 0xFFBCD4F6);
        graphics.horizontalLine(actionBounds.x(), actionBounds.right() - 1, actionBounds.y(), 0xFF8EADE0);
    }

    public void renderStartMenuHeader(GuiGraphicsExtractor graphics, Font font, UiRect bounds, UiRect avatarBounds, String playerName, Identifier playerSkin) {
        assets.fillHorizontalGradient(graphics, bounds.x(), bounds.y(), bounds.right(), bounds.bottom(), 0xFF1A4EA8, 0xFF4E8FE3);
    ...
    public void renderStartMenuRightColumnPane(GuiGraphicsExtractor graphics, UiRect bounds) {
        ...
        assets.fillVerticalGradient(graphics, bounds.x() + 1, bounds.y() + 1, bounds.right() - 1, bounds.bottom() - 1, 0xFFEFF4FF, 0xFFD4E1F7);
    }
    ...
    public void renderStartMenuCategoryRow(GuiGraphicsExtractor graphics, Font font, UiRect bounds, String text, boolean selected, boolean hovered) {
        if (selected || hovered) {
            assets.fillHorizontalGradient(graphics, bounds.x(), bounds.y(), bounds.right(), bounds.bottom(), 0xFF2A64C2, 0xFF4D95F2);
        }
    ...
    public void renderStartMenuModuleRow(GuiGraphicsExtractor graphics, Font font, UiRect bounds, String text, boolean enabled, boolean highlighted) {
        ...
        graphics.fill(bounds.x() + 4, bounds.y() + 4, bounds.x() + 10, bounds.bottom() - 4, enabled ? 0xFF36A12B : 0xFFAAB7CC);
        if (enabled) {
    ...
    public void renderStartMenuFlyoutShell(GuiGraphicsExtractor graphics, Font font, UiRect bounds, String title) {
        ...
        assets.drawRaisedBox(graphics, bounds.x() + 1, bounds.y() + 1, bounds.right() - 1, bounds.bottom() - 1, 0xFFF9F6EC);
        assets.fillHorizontalGradient(graphics, bounds.x() + 2, bounds.y() + 2, bounds.right() - 2, bounds.y() + 18, 0xFF2B66C5, 0xFF4A8EEA);
        graphics.text(font, title, bounds.x() + 8, bounds.y() + 7, 0xFFFFFFFF, false);
    }

    public void renderStartMenuFlyoutRow(GuiGraphicsExtractor graphics, Font font, UiRect bounds, String text, boolean enabled, boolean hovered) {
    ...
    private void renderStartMenuAvatar(GuiGraphicsExtractor graphics, UiRect avatarBounds, Identifier playerSkin) {
        assets.drawSunkenBox(graphics, avatarBounds.x() - 1, avatarBounds.y() - 1, avatarBounds.right() + 1, avatarBounds.bottom() + 1, 0xFFFFFFFF);
        if (playerSkin == null) {
            graphics.fill(avatarBounds.x(), avatarBounds.y(), avatarBounds.right(), avatarBounds.bottom(), 0xFFB7D0F6);
            graphics.fill(avatarBounds.x() + 6, avatarBounds.y() + 7, avatarBounds.x() + 9, avatarBounds.y() + 10, 0xFF1B386F);
            graphics.fill(avatarBounds.x() + 15, avatarBounds.y() + 7, avatarBounds.x() + 18, avatarBounds.y() + 10, 0xFF1B386F);
            graphics.fill(avatarBounds.x() + 7, avatarBounds.y() + 15, avatarBounds.x() + 17, avatarBounds.y() + 17, 0xFF1B386F);
            return;
        }
    ...
    private void renderStartMenuActionButton(GuiGraphicsExtractor graphics, Font font, UiRect bounds, String text, boolean hovered) {
        ...
        assets.fillVerticalGradient(graphics, bounds.x() + 1, bounds.y() + 1, bounds.right() - 1, bounds.bottom() - 1, top, bottom);
